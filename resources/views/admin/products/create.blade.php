<x-admin-layout>
    <x-slot:header>Adicionar Departamento de Produto</x-slot>
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row gap-6">
        @csrf
        <div class="flex-1 space-y-4">
            <input type="text" name="name" required placeholder="Nome do Departamento (ex: Frutas e Legumes)" class="w-full border-gray-300 rounded-xl text-lg px-4 py-3 focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50 transition-colors shadow-sm">
        ...
        <div class="w-full lg:w-[280px] space-y-6">
            ...
            <div class="bg-white border border-gray-100 shadow-md rounded-xl p-6">
                ...
                <p class="text-[10px] text-gray-400 mt-2">Selecione múltiplas fotos para o carrossel da página.</p>
            </div>
            ...
            <div class="bg-white border border-gray-100 shadow-md rounded-xl overflow-hidden">
                ...
                <div class="bg-gray-50/50 p-4 flex justify-end">
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-5 rounded-lg shadow-sm transition-all duration-200 w-full text-center">Criar Categoria</button>
                </div>
            </div>
        </div>
    </form>
