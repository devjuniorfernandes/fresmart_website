<x-admin-layout>
    ...
    <x-slot:actions>
        <a href="{{ route('admin.services.create') }}" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition-all duration-200 text-sm">Adicionar Novo</a>
    </x-slot>
            ...
            <tbody>
                @forelse($services ?? [] as $service)
                    <tr class="border-b border-gray-50 hover:bg-[#f6f7f7]">
                        <td class="py-3 px-3"><strong class="text-[#2271b1]">{{ $service->name }}</strong></td>
                        <td class="py-3 px-3">{{ $service->created_at->format('d/m/Y') }}</td>
                            <td class="py-4 px-4 align-top">
                                <a href="{{ route('admin.services.edit', $service->id) }}" class="text-blue-600 hover:text-blue-800 font-medium transition-colors mr-2">Editar</a>
                ...
                @empty
                    <tr><td colspan="2" class="py-4 px-3 text-center text-gray-500">Nenhum serviço.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
