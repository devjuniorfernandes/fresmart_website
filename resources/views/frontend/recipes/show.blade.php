<x-frontend.layout>
    ...
    @endif
    <x-frontend.page-header :title="$recipe->title" :subtitle="$recipe->category . ' | ' . $recipe->prep_time_minutes . ' min'" :image="$recipe->image ? (str_starts_with($recipe->image, 'uploads/') ? $recipe->image : 'storage/'.$recipe->image) : 'assets/img/receita1.jpg'" />
