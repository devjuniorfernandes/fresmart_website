...
<a href="{{ route('services.show', $service) }}" class="block rounded-[16px] overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 group relative">
    <div class="w-full aspect-[16/10] md:aspect-[4/3]">
        <img src="{{ $service->image ? asset(str_starts_with($service->image, 'uploads/') ? $service->image : 'storage/'.$service->image) : asset('placeholder.png') }}" alt="{{ $service->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
    </div>
