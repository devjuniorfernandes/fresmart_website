    ...
    <div class="bg-[#45B500] w-full">
        <div class="max-w-[1528px] mx-auto px-6 lg:px-10 h-[76px] flex items-center gap-6">
            ...
            <a href="{{ route('home') }}" class="flex-shrink-0">
                <img src="{{ $settings->logo ? asset($settings->logo) : asset('assets/img/logo-white.png') }}"
                    alt="FRESMART" class="h-14 w-auto">
            </a>

            <!-- Desktop Nav Links -->
            <div class="hidden md:flex items-stretch h-full font-bold text-white text-[13px] tracking-wide">
                <a href="{{ route('home') }}"
                    class="uppercase flex items-center px-4 border-b-[3px] {{ request()->routeIs('home') ? 'border-white' : 'border-transparent' }} hover:border-white/60 hover:bg-white/10 transition whitespace-nowrap">INÍCIO</a>

                <!-- Products Hover Dropdown -->
                <div class="relative group flex items-stretch">
                    <a href="{{ route('products.index') }}"
                        class="uppercase flex items-center px-4 border-b-[3px] {{ request()->routeIs('products.*') ? 'border-white' : 'border-transparent' }} hover:border-white/60 hover:bg-white/10 transition gap-1.5 whitespace-nowrap">
                        PRODUTOS <i class="fas fa-chevron-down text-[9px]"></i>
                    </a>
                    @if ($headerProducts->isNotEmpty())
                        <div
                            class="absolute top-full left-0 pt-1 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                            <div
                ...
                <div class="relative group flex items-stretch">
                    <a href="{{ route('services.index') }}"
                        class="uppercase flex items-center px-4 border-b-[3px] {{ request()->routeIs('services.*') ? 'border-white' : 'border-transparent' }} hover:border-white/60 hover:bg-white/10 transition gap-1.5 whitespace-nowrap">
            ...
            <div class="hidden md:flex items-center gap-5">
                <div class="flex items-center gap-4 text-white/80 text-base">
                    @if ($settings->facebook)
                        <a href="{{ $settings->facebook }}" target="_blank" class="hover:text-white transition"><i
                                class="fab fa-facebook-f"></i></a>
                    @endif
                    @if ($settings->instagram)
                        <a href="{{ $settings->instagram }}" target="_blank" class="hover:text-white transition"><i
                                class="fab fa-instagram"></i></a>
                    @endif
                    @if ($settings->tiktok)
                        <a href="{{ $settings->tiktok }}" target="_blank" class="hover:text-white transition"><i
                                class="fab fa-tiktok"></i></a>
                    @endif
                    @if ($settings->linkedin)
                        <a href="{{ $settings->linkedin }}" target="_blank" class="hover:text-white transition"><i
                                class="fab fa-linkedin-in"></i></a>
                    @endif
                    @if ($settings->youtube)
                        <a href="{{ $settings->youtube }}" target="_blank" class="hover:text-white transition"><i
                ...
                <div class="flex items-center gap-1 text-white text-[12px] font-bold tracking-wide">
                    ...
                    @else
                        @php
                            ...
                            $prefix = $isHome ? '' : url('/');
                        @endphp
                        <a href="{{ $prefix }}#ofertas"
                            class="bg-white text-[#3a9900] px-4 py-1.5 rounded-lg hover:bg-white/90 transition font-extrabold uppercase">OFERTAS
                            ESPECIAIS</a>
                        <a href="{{ $prefix }}#receitas"
                            class="hover:bg-white/20 px-4 py-1.5 rounded-lg transition uppercase">RECEITAS</a>
                        <a href="{{ $prefix }}#servicos"
                            class="hover:bg-white/20 px-4 py-1.5 rounded-lg transition uppercase">NOSSOS SERVIÇOS</a>
                        <a href="{{ $prefix }}#lojas"
                            class="hover:bg-white/20 px-4 py-1.5 rounded-lg transition uppercase">NOSSAS LOJAS</a>
                    @endif
                </div>
            </div>
        </div>
